...
public class ThreadDijkstra {
	int[] potentialRooms_;
	int potentialRoomSize_, rootId_, endId_;
	Resultat res_;

	public ThreadDijkstra(int[] potentialRooms, int potentialRoomSize, int rootId, int endId){
		potentialRooms_ = potentialRooms;
		potentialRoomSize_ = potentialRoomSize;
		rootId_ = rootId;
		endId_ = endId;
	}
	...
	public void ThreadLoop() {
		res_.path = AI.Dijkstra(new Graph(potentialRooms_, potentialRoomSize_), rootId_, endId_, out res_.length);
	}
}
...
public class AI : MonoBehaviour
{
	...
	/// </summary>
    private float monsterThreshold = 0.2f;
    ...
    private List<Vector2> FindClosestRoomPath(List<Vector2> eligibleRooms)
    {
		...
		List<ThreadDijkstra> results = new List<ThreadDijkstra>();
        ...
        foreach (Vector2 eligibleRoom in eligibleRooms) // Determining the closest room
        {
			...
			*/
			...
			/*path = AI.Dijkstra(new Graph(potentialRooms, potentialRoomSize), rootId, endId, out length);
			...
			// MultiThread approch
			ThreadDijkstra td = new ThreadDijkstra(potentialRooms, potentialRoomSize, rootId, endId);
			results.Add(td);
			Thread t = new Thread (new ThreadStart (td.ThreadLoop));
			lt.Add (t);
			t.Start();
        }
		...
		foreach (ThreadDijkstra td in results) {
			if (td.getResult().length < minLength) // updating the global minimum distance and optimal path
			{
				minLength = td.getResult().length;
				minPath = td.getResult().path;
			}
		}
    ...
    public static List<Vector2> Dijkstra(Graph G, int idRoot, int idEnd, out int length)
    {
        ...
        {
            Node s1 = AI.findminScore(remainingNodes); // Gets the node with the min score
	...
	public static Node findminScore(List<Node> nodes)
    {
    ...
    public static void updateScores(Node s1)
    {
